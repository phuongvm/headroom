...
<Callout type="warning" title="Claude Code auto mode">
LiteLLM and other schema-translating routes are not claimed to preserve Claude
Code's server-side auto-mode classifier protocol. Requests continue to fail
open as ordinary model traffic, but Claude Code may use its billable
client-side classifier fallback. Native Anthropic transport is the supported
path until an adapter has an end-to-end safeguard fixture.
</Callout>
