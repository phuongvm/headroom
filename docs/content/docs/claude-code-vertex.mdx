...
- Claude Code's server-side auto-mode classifier is supported only by the
  native Anthropic-compatible wire path. The Vertex translation path remains
  fail-open but may use Claude Code's billable client-side classifier fallback
  until a provider-native safeguard fixture is verified.
