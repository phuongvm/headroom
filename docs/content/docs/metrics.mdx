...
# HELP headroom_requests_failed_total Requests that failed upstream (4xx and 5xx, excluding 429), by provider
headroom_requests_failed_total{provider="anthropic"} 3

# HELP headroom_requests_rate_limited_total Requests rejected with 429, by who rejected them (headroom=our own limiter, upstream=the provider)
headroom_requests_rate_limited_total{source="headroom"} 0
headroom_requests_rate_limited_total{source="upstream"} 12
```

<Callout type="warn" title="requests_total is completed requests, not all traffic">
`headroom_requests_total` counts requests that **completed**. A request that ended in a 4xx,
5xx or 429 lands in `headroom_requests_failed_total` / `headroom_requests_rate_limited_total`
and is not in `requests_total` — so a failure rate must add `failed` back into its own
denominator. `headroom_inbound_requests_total` is the all-traffic counter.

`headroom_requests_failed_total` gained a `provider` label and
`headroom_requests_rate_limited_total` a `source` label, so unaggregated queries on them now
return several series. `sum without (provider) (...)` and `sum without (source) (...)`
reproduce the previous single-series behaviour.
</Callout>
...
| `headroom_requests_failed_total{provider}` | Reliability — upstream **4xx and 5xx** (429s are counted separately), attributed to the provider that failed | 0 |
| `headroom_requests_rate_limited_total{source}` | Throttling — `source="headroom"` is Headroom's own rate limiter, `source="upstream"` is the provider's | 0 |
